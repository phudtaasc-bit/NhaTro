...
function NT_taoTongCongVaDashboard_(sheet, monthDate) {
  ...
  // Google Sheets không cho phép hợp nhất vùng cắt qua ranh giới cột/hàng cố định.
  // Tạm bỏ cố định trước khi tạo dashboard, sau đó khôi phục lại.
  ...
  try {
    ...
    const dashboard = [
      ...
      [
        ...
        ['Khách chưa đăng ký', '=COUNTIF(J' + NT.MONTH_DATA_ROW + ':J' + lastDataRow + sep + '"Chưa đăng ký")'],
        ['Khách hết hạn', '=COUNTIF(J' + NT.MONTH_DATA_ROW + ':J' + lastDataRow + sep + '"Hết hạn")']
      ]
    ];
    ...
    sheet.setFrozenRows(Math.max(frozenRows, NT.MONTH_HEADER_ROW));
    sheet.setFrozenColumns(Math.max(frozenCols, 3));
  }
}
